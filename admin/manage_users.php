                                    ...
                                    <?php if (!empty($users)): ?>
                                        ...
                                        ?>
                                            <tr class="bg-white border-b hover:bg-gray-50 transition-colors duration-150">
                                                ...
                                                <td class="px-6 py-4">
                                                    <span class="px-3 py-1.5 text-xs font-semibold leading-tight rounded-full <?php echo $role_info['class']; ?> inline-flex items-center whitespace-nowrap">
                                ...
                                ?>
                                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                        ...
                                        <div class="flex items-center mb-3">
                                            <i class="fas fa-user-circle text-gray-400 text-2xl ml-2"></i>
                                            <h4 class="font-bold text-gray-900 text-lg"><?php echo htmlspecialchars($user['full_name']); ?></h4>
                                        </div>

                                        <!-- Email -->
                                        <div class="flex items-start mb-2">
                                            <i class="fas fa-envelope text-gray-400 ml-2 mt-1"></i>
                                            <span class="text-gray-600 text-sm break-all"><?php echo htmlspecialchars($user['email']); ?></span>
                                        </div>

                                        <!-- User Type -->
                                        <div class="flex items-center mb-2">
                                            <i class="fas fa-tag text-gray-400 ml-2"></i>
                                            <span class="px-3 py-1 text-xs font-semibold rounded-full <?php echo $role_info['class']; ?> inline-flex items-center">
                                                <i class="fas <?php echo $role_info['icon']; ?> ml-1"></i>
                                                <?php echo htmlspecialchars($role_info['text']); ?>
                                            </span>
                                        </div>
                                        ...
                                        <div class="flex items-center mb-3">
                                            <i class="fas fa-calendar text-gray-400 ml-2"></i>
                                            <span class="text-gray-600 text-sm"><?php echo date('d-m-Y', strtotime($user['created_at'])); ?></span>
                                        </div>

                                        <!-- Actions -->
                                        <div class="flex gap-2 pt-3 border-t border-gray-300">
                                            <a href="edit_user.php?id=<?php echo $user['id']; ?>" class="flex-1 text-center bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-all duration-200 flex items-center justify-center gap-2">
                                                <i class="fas fa-edit"></i>
                                                <span>تعديل</span>
                                            </a>
                                            <a href="delete_user.php?id=<?php echo $user['id']; ?>" class="flex-1 text-center bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition-all duration-200 flex items-center justify-center gap-2" onclick="return confirm('هل أنت متأكد من رغبتك في حذف هذا المستخدم؟');">
